...
'''Cell 9'''
X_train, X_test = train_test_split(train_data, test_size=0.3, random_state=42)
#entity_splitting train and test subsets
gss = GroupShuffleSplit(n_splits = 1, train_size = 0.7, random_state = 42)
groups = train_data['ENTITY_ID'].astype(str)
le = LabelEncoder()
...
no_shared_groups = train_groups.isdisjoint(X_test_entity['ENTITY_ID'])
...
def lgb_5fold(X_train,y_train, X_test, valid, train_data, seed, features_slc, entity = False, groups = None):
    ...
    if not entity:
        KF = StratifiedKFold(n_splits=4, random_state=seed, shuffle=True) #4 not 5, to make this comparable to the entity splitting kfold
        KF_splits = KF.split(X_train.values, y_train.values)
    else:
        df = pd.DataFrame({'group': groups, 'label': y_train.values})
        print(df.groupby(['group', 'label']).size().unstack(fill_value=0))

        KF = StratifiedGroupKFold(n_splits=4) # there are only 4 groups (entity ID's) with label 0. To stratify the kfolds, I am limited to a 4-fold split.
    ...
    predictions_lgb = np.zeros((len(X_test), 4))
    valid_lgb = np.zeros((len(valid), 4))
    train_lgb = np.zeros((len(train_data), 4))
...
oof_lgb,predictions_lgb,valid_lgb,train_lgb,df_feature_importance = lgb_5fold(X_train, y_train, X_test, valid, train_data, 12, features_slc)
oof_lgb_ent,predictions_lgb_ent,valid_lgb_ent,train_lgb_ent,df_feature_importance_ent = lgb_5fold(X_train_entity, y_train_entity, X_test_entity, valid, train_data, 12, features_slc, entity=True, groups = X_train_entity['ENTITY_ID'])
