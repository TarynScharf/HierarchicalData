...
def evaluate_model_with_shap(x_test, y_test, model,xtrain,CL, shap_output_location):
    y_pred = model.predict(x_test)
    y_test.reset_index(inplace=True, drop=True)
    results = pd.concat([y_test, pd.DataFrame(y_pred)], axis=1)
    results.columns=['actual', 'predicted']
    ...
    if CL:
        ...
        sns.scatterplot(data=pc1_df, x='oscillatory_zonation', y='shaposcillatory_zonation',label = 'OZ')
        sns.scatterplot(data=pc1_df, x='sector_zonation', y='shapsector_zonation', label = 'SZ')
        sns.scatterplot(data=pc1_df, x='homogenous_zonation', y='shaphomogenous_zonation', label = 'HZ')
        sns.scatterplot(data=pc1_df, x='PC1', y='shapPC1', label='PC1')
        sns.scatterplot(data=pc1_df, x='PC2', y='shapPC2', label='PC2')
        sns.scatterplot(data=pc1_df, x='PC3', y='shapPC3', label='PC3')
    else:
        shap_values_df = pd.DataFrame(shap_values.values, columns=['shapPC1', 'shapPC2', 'shapPC3'])
        data_values_df = pd.DataFrame(shap_values.data, columns=['PC1', 'PC2', 'PC3'])
        pc1_df = pd.concat([data_values_df, shap_values_df], axis=1)
        sns.scatterplot(data = pc1_df, x='PC1', y='shapPC1', label = 'PC1')
        sns.scatterplot(data = pc1_df, x='PC2', y='shapPC2', label = 'PC2')
        sns.scatterplot(data = pc1_df, x='PC3', y='shapPC3',label = 'PC3')
...
def train_test_model(input_data_filepath, aggregate_size, resampling_repeats, test_split_size, epochs,splitting_function,kfold_function,n_folds, batchsize= None):
    #project description
    description = f'Kfold_{aggregate_size}AGGx{resampling_repeats}Resample_SHAPE_UTH_CL'
    ...
    df1 = load_dataset(input_data_filepath,columns)
    ...
    train_OG, test_OG = splitting_function(dataframe=df, test_split_size=test_split_size, stratify_column='bin', columns = columns)
    ...
    #enumerate(kfold.split(x_train_pca, y_train_pca)):
    for i, (train_index, test_index) in enumerate(kfold_function(n_folds,df_train_pca)):
        xtrain, xval = df_train_pca.iloc[train_index][fields], df_train_pca.iloc[test_index][fields]
        ytrain, yval = df_train_pca.iloc[train_index]['SiO2_pct'], df_train_pca.iloc[test_index]['SiO2_pct']
...
def observation_split_strategy(dataframe, test_split_size, stratify_column, columns):
    #x = dataframe.loc[:, columns]
    #y = dataframe[[stratify_column]]
    return train_test_split(dataframe, test_size=test_split_size, stratify=dataframe['bin'])

def entity_split_strategy (dataframe, test_split_size, stratify_column,columns):
    entity_identifier = 'GSWA_sample_id'
    #df = dataframe.loc[:,columns]
    return custom_stratified_train_test_split(dataframe=dataframe, sample_id_column=entity_identifier, stratify_column=stratify_column,fraction=test_split_size)

def observation_kfold(number_of_splits,dataframe ):

    x_data = dataframe
    y_data = dataframe[['SiO2_pct']]

    kfold = KFold(n_splits=number_of_splits, shuffle=True)

    return kfold.split(x_data, y_data)

def entity_kfold(number_of_splits, dataframe):
    entity_identifier = 'GSWA_sample_id'
    stratify_column = 'bin'

    return custom_stratified_kfold(dataframe,entity_identifier, stratify_column, n_splits=number_of_splits)

def run_experiment(splitting_strategy, kfold_strategy):

    all_metrics = []
    all_results = []

    #I will repeat the train-test procedure 10 times
    for i in range(10):
        results, metrics  = train_test_model(
            input_data_filepath=os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),'Data_files','Dataset.csv'),
            ...
            splitting_function =splitting_strategy,
            kfold_function = kfold_strategy,
            n_folds = 10,
            batchsize=32, #batchsize of 32 is the Tensorflow default
            )

        all_metrics.extend(pd.concat(metrics, axis=0)['mse'].values)
        all_results.extend(results)

    return all_metrics, all_results

entity_metrics, entity_results = run_experiment(entity_split_strategy,entity_kfold)
entity_results_df = pd.concat(entity_results)
entity_results_df.to_csv('entity_results_df.csv')
observation_metrics,observation_results  = run_experiment(observation_split_strategy,observation_kfold)
observation_results_df = pd.concat(observation_results)
observation_results_df.to_csv('observation_results.csv')
plot_cv_results(entity_metrics, observation_metrics, 'Cross-validation results')
linear_regression_of_prediction_results(entity_results_df, observation_results_df)
