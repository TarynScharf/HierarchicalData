...
'''
Hypothesis: 
    Geoscience data is often genetically structured.When datasets are hierarchically structured such that observations 
    are linked to entities by means of latent variables, splitting by observation allows for data leakage. This results 
    in inflated performance metrics during model testing, which are not reproduced when the model is applied to new data.
Test:
    Create multiple entities and test model performance with observation-level and entity-level data splitting.
    The test is repeated multiple times and the resultant MSE values are presented in violin-plot format, and compared using
     K-S test.
'''
